...
with st.expander("② إعدادات التأخير", expanded=True):
    ...
    cfg["tardiness_base"] = st.radio(
        ...
        format_func=lambda x: {
            ...
            "after_grace": f"من بعد وقت السماح ({cfg['shift_start']} + {cfg['grace_minutes']} د)    ",
        }[x],
    ...
    cfg["tardiness_rounding"] = st.radio(
        ...
        format_func=lambda x: {
            "daily": "تقريب يومي — كل يوم يُقرَّب منفرداً ",
            "total": "تقريب تراكمي — يجمع الدقائق أولاً ثم يقرّب مرة واحدة ",
...
with st.expander("④ إعدادات الإضافي", expanded=True):
    ...
    cfg["overtime_base"] = st.radio(
        ...
        format_func=lambda x: {
            "shift_end":       f"من نهاية الدوام ({cfg['shift_end']})   ",
            "after_threshold": f"من بعد حد الإضافي ({cfg['shift_end']} + {cfg['overtime_threshold_minutes']} د)  ",
        }[x],
